...
| 属性 | 必填 | 默认值 | 说明 |
|------|------|--------|------|
| `kecong:authCode` | 否 | 内置默认值 | 协议认证码。每个控制器可能有独立的认证码（联系科聪销售获取），未配置时使用内置默认值 |
...
2. 检查认证码是否正确 — 若配置了 `kecong:authCode`，请确认与科聪提供的该控制器专属认证码一致；未配置时使用内置默认值
3. 确认机器人处于正确的导航模式（激光 vs 二维码）
4. 确认导航程序版本 ≥ V3.1.18

### "认证码错误" (0xFF)

协议认证码不匹配。若配置了 `kecong:authCode`，请确保使用的是科聪为**该控制器**提供的认证码（每个控制器可能有独立的认证码）。若使用内置默认值仍报此错误，需联系科聪销售获取正确的认证码。
